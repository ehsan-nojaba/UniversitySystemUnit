...
public class PreRegistrationSubmitAndDemandIntegrationTests : IClassFixture<WebApplicationFactory<Program>>
{
    private readonly WebApplicationFactory<Program> _factory;
    private readonly HttpClient _client;
    private readonly ITokenService _tokenService;

    public PreRegistrationSubmitAndDemandIntegrationTests(WebApplicationFactory<Program> factory)
    {
        _factory = factory;
    ...
    private async Task<(User User, Student Student, AcademicTerm Term, Course Course1, Course Course2, Course Course3)>
    ...
    public async Task Draft_CanBeSubmitted_Successfully()
    {
        ...
        using var scope = _factory.Services.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<IApplicationDbContext>();
        var preReg = await context.StudentPreRegistrations
            .FirstOrDefaultAsync(pr => pr.Id == result.PreRegistrationId);
