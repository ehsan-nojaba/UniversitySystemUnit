    ...
    IDateTimeProvider dateTimeProvider) : IStudentPreRegistrationService
{
    private async Task<Student> GetCurrentStudentAsync(CancellationToken cancellationToken)
    {
        ...
        if (student is null) throw new NotFoundException("پروفایل دانشجویی برای کاربر جاری یافت نشد.");
        return student;
    }
    ...
    private async Task<AcademicTerm> GetActiveTermAsync(long termId, CancellationToken cancellationToken)
    {
        ...
        if (term is null) throw new NotFoundException(nameof(AcademicTerm), termId);
    ...
    public async Task<StudentPreRegistrationDto> SaveDraftAsync(long academicTermId, ICollection<SelectedCourseItemDto> courses, CancellationToken cancellationToken = default)
    {
        ...
        if (preRegistration is null)
        {
            ...
            foreach (var item in courses) preRegistration.AddCourse(item.CourseId, item.Priority);
        }
        else
        {
            ...
            foreach (var toRemove in itemsToRemove) preRegistration.RemoveCourse(toRemove.CourseId);
            ...
            foreach (var item in courses)
            {
                ...
                {
                    if (existingItem.Priority != item.Priority) preRegistration.UpdateCoursePriority(item.CourseId, item.Priority);
                }
                else preRegistration.AddCourse(item.CourseId, item.Priority);
            }
        }
        ...
        var responseCourses = (
            from c in courses
            orderby c.Priority
            let info = eligibleMap[c.CourseId]
            select new PreRegistrationCourseItemDto { CourseId = c.CourseId, Code = info.Code, Title = info.Title, Credits = info.Credits, Priority = c.Priority }
        ).ToList();
    ...
    public async Task<StudentPreRegistrationDto> SubmitAsync(long academicTermId, CancellationToken cancellationToken = default)
    {
        ...
        if (preRegistration is null) throw new NotFoundException("پیش‌ثبت‌نامی برای این ترم تحصیلی یافت نشد.");
        if (preRegistration.Status != RequestStatus.Draft) throw new BusinessException("فقط پیش‌ثبت‌نام در وضعیت پیش‌نویس (Draft) قابل ثبت نهایی است.");
        if (preRegistration.Items.Count == 0) throw new BusinessException("برای ثبت نهایی، باید حداقل یک درس انتخاب شده باشد.");
        ...
        var courseItems = (
            from i in preRegistration.Items
            orderby i.Priority
            select new PreRegistrationCourseItemDto { CourseId = i.CourseId, Code = i.Course.Code, Title = i.Course.Title, Credits = i.Course.Credits, Priority = i.Priority }
        ).ToList();
    ...
    public async Task<StudentPreRegistrationDto?> GetByTermAsync(long academicTermId, CancellationToken cancellationToken = default)
    {
        var student = await GetCurrentStudentAsync(cancellationToken);
        var preRegistration = await repository.GetPreRegistrationWithItemsAndCoursesAsync(student.Id, academicTermId, cancellationToken);
        if (preRegistration is null) return null;
