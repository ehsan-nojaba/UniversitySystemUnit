    ...
    : AbstractValidator<SaveStudentPreRegistrationCommand>
{
    public SaveStudentPreRegistrationCommandValidator()
    {
        ...
        RuleFor(x => x.Courses)
            .NotNull()
            .WithMessage("لیست درس‌ها نمی‌تواند خالی باشد.")
            .Must(c => c != null && c.Count > 0)
            .WithMessage("حداقل یک درس باید برای پیش‌ثبت‌نام انتخاب شود.")
            .Must(HaveUniqueCourseIds)
            .WithMessage("انتخاب درس‌های تکراری مجاز نیست.");
        ...
        RuleForEach(x => x.Courses).ChildRules(course =>
        {
            ...
            course.RuleFor(c => c.Priority)
                .GreaterThan(0)
                .WithMessage("اولویت درس باید بزرگتر از صفر باشد.");
        });
    }
