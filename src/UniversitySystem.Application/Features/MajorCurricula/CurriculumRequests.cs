...
public sealed record CreateMajorCourseCommand(long MajorId, string Code, string Title, int Credits, int RecommendedTerm, bool IsRequired) : IRequest<MajorCurriculumDto>;

/// <summary>عملیات چارت را بدون دسترسی مستقیم به دیتابیس به سرویس مربوط می‌سپارد.</summary>
public sealed class CurriculumRequestHandler(MajorCurriculumService service) : IRequestHandler<GetMajorsQuery, ICollection<MajorOptionDto>>, IRequestHandler<GetMajorCurriculumQuery, MajorCurriculumDto>, IRequestHandler<SaveMajorCurriculumCommand, MajorCurriculumDto>, IRequestHandler<CreateMajorCourseCommand, MajorCurriculumDto>
{
    ...
    public Task<MajorCurriculumDto> Handle(CreateMajorCourseCommand r, CancellationToken ct) => service.CreateCourseAsync(r.MajorId, new(r.Code, r.Title, r.Credits, r.RecommendedTerm, r.IsRequired), ct);
}
