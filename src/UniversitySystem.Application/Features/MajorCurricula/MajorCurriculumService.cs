...
public sealed class MajorCurriculumService(IMajorCurriculumRepository repository, IUnitOfWork unitOfWork)
{
    ...
    public Task<MajorCurriculumDto> SaveAsync(long majorId, ICollection<CurriculumCourseInput> courses, CancellationToken ct) => repository.ExecuteSerializableAsync(async () =>
    {
    ...
    private static MajorCurriculumDto Map(Major major, Curriculum? curriculum, ICollection<Course>? definitions = null) => new(major.Id, major.Title, curriculum?.Id, curriculum?.Title, curriculum?.Version, curriculum?.CurriculumCourses.OrderBy(c => c.RecommendedTerm).ThenBy(c => c.CourseId).Select(c =>
    {
