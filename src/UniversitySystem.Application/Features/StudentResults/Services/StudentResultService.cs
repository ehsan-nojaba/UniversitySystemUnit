...
/// </summary>
public sealed class StudentResultService(IStudentPreRegistrationRepository studentRepository,
    ICourseOfferingRepository offeringRepository, IEnrollmentRepository enrollmentRepository, ICurrentUserService currentUser)
{
    public async Task<StudentResultDto> GetAsync(long termId, CancellationToken cancellationToken)
    {
        ...
        var registration = await studentRepository.GetPreRegistrationWithItemsAndCoursesAsync(student.Id, termId, cancellationToken)
            ?? throw new NotFoundException("Pre-registration was not found.");
        if (registration.Status != RequestStatus.Submitted) throw new BusinessException("Pre-registration must be submitted.");
        ...
        foreach (var item in registration.Items.OrderBy(i => i.Priority))
        {
            ...
            foreach (var candidate in allOfferings.Where(o => o.CourseId == item.CourseId))
            {
                ...
                offerings.Add(new(offering.Id, offering.Capacity, Math.Max(0, offering.Capacity - count), offering.IsActive,
                    offering.TeachingAssignments.Select(a => new ProfessorResultDto(a.ProfessorId, a.Professor.User.FirstName + " " + a.Professor.User.LastName)).ToList(),
                    offering.Schedules.OrderBy(s => s.DayOfWeek).ThenBy(s => s.StartTime).Select(s => new ScheduleResultDto(s.DayOfWeek, s.StartTime, s.EndTime)).ToList()));
            }
