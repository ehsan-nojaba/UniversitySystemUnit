...
{
    public async Task<IReadOnlyCollection<FinalTeachingOffering>> GetMyFinalScheduleAsync(long termId, CancellationToken ct)
    {
    ...
    public async Task<IReadOnlyCollection<ProfessorCourseOption>> GetMyCoursesAsync(long termId, CancellationToken ct)
    {
        if (!long.TryParse(currentUser.UserId, out var userId)) { throw new UnauthorizedAccessException(); }
        var professorId = await repository.GetProfessorIdAsync(userId, ct) ?? throw new NotFoundException("پروفایل استاد پیدا نشد.");
        if (!await repository.TermExistsAsync(termId, ct)) { throw new NotFoundException("ترم پیدا نشد."); }
    ...
    public async Task<IReadOnlyCollection<long>> GetAllowedAsync(long professorId, CancellationToken ct)
    {
    ...
    public async Task<IReadOnlyCollection<long>> SaveAllowedAsync(long professorId, IReadOnlyCollection<long> ids, CancellationToken ct)
    {
