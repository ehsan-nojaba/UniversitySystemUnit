...
public sealed class UiSupportRepository(ApplicationDbContext _context) : IUiSupportRepository
{
    ...
    public async Task<ICollection<CourseOptionDto>> GetCoursesAsync(CancellationToken cancellationToken) => await _context.Courses.AsNoTracking().Where(c => c.IsActive).OrderBy(c => c.Code).Select(c => new CourseOptionDto(c.Id, c.Code, c.Title, c.Credits)).ToListAsync(cancellationToken);
