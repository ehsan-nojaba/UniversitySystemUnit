...
{
    public async Task<IReadOnlyCollection<AcademicTermOptionDto>> GetTermsAsync(CancellationToken cancellationToken) => await _context.AcademicTerms.AsNoTracking().OrderByDescending(t => t.StartDate).ThenBy(t => t.Id).Select(t => new AcademicTermOptionDto(t.Id, t.Code, t.Title, t.IsActive, t.StartDate, t.EndDate)).ToListAsync(cancellationToken);
    public async Task<IReadOnlyCollection<CourseOptionDto>> GetCoursesAsync(CancellationToken cancellationToken) => await _context.Courses.AsNoTracking().Where(c => c.IsActive).OrderBy(c => c.Code).Select(c => new CourseOptionDto(c.Id, c.Code, c.Title, c.Credits)).ToListAsync(cancellationToken);
    public async Task<IReadOnlyCollection<ProfessorOptionDto>> GetProfessorsAsync(CancellationToken cancellationToken) => await _context.Professors.AsNoTracking().Where(p => p.User.IsActive).OrderBy(p => p.User.LastName).ThenBy(p => p.Id).Select(p => new ProfessorOptionDto(p.Id, p.PersonnelCode, p.User.FirstName + " " + p.User.LastName)).ToListAsync(cancellationToken);
