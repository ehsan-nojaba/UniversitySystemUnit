...
{
    public async Task<ICollection<AcademicTermOptionDto>> GetTermsAsync(CancellationToken cancellationToken) => await _context.AcademicTerms.AsNoTracking().Where(t => t.IsActive).OrderByDescending(t => t.StartDate).ThenBy(t => t.Id).Select(t => new AcademicTermOptionDto(t.Id, t.Code, t.Title, t.IsActive, t.StartDate, t.EndDate)).ToListAsync(cancellationToken);
    public async Task<ICollection<CourseOptionDto>> GetCoursesAsync(long? majorId, CancellationToken cancellationToken) => await _context.Courses.AsNoTracking().Where(c => c.IsActive && (majorId == null || c.CurriculumCourses.Any(cc => cc.Curriculum.MajorId == majorId))).OrderBy(c => c.Code).Select(c => new CourseOptionDto(c.Id, c.Code, c.Title, c.Credits)).ToListAsync(cancellationToken);
    public async Task<ICollection<ProfessorOptionDto>> GetProfessorsAsync(CancellationToken cancellationToken) => await _context.Professors.AsNoTracking().Where(p => p.User.IsActive).OrderBy(p => p.User.LastName).ThenBy(p => p.Id).Select(p => new ProfessorOptionDto(p.Id, p.PersonnelCode, p.User.FirstName + " " + p.User.LastName)).ToListAsync(cancellationToken);
