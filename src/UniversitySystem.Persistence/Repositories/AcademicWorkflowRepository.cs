...
{
    public async Task<IReadOnlyCollection<FinalTeachingOffering>> GetFinalScheduleAsync(long professorId, long termId, CancellationToken ct)
    {
    ...
    public async Task<IReadOnlyCollection<long>> GetAllowedCourseIdsAsync(long professorId, CancellationToken ct) => await _context.ProfessorCourses.AsNoTracking().Where(p => p.ProfessorId == professorId).Select(p => p.CourseId).ToListAsync(ct);
    public async Task<IReadOnlyCollection<ProfessorCourseOption>> GetCoursesAsync(long professorId, long termId, CancellationToken ct)
    {
    ...
    public async Task<bool> CoursesExistAsync(IReadOnlyCollection<long> ids, CancellationToken ct) => await _context.Courses.CountAsync(c => ids.Contains(c.Id) && c.IsActive, ct) == ids.Count;
    public async Task<bool> HasCommittedCoursesAsync(long professorId, IReadOnlyCollection<long> ids, CancellationToken ct)
    {
    ...
    public async Task ReplaceCoursesAsync(long professorId, IReadOnlyCollection<long> ids, CancellationToken ct)
    {
        ...
        {
            _context.ProfessorCourses.Add(new ProfessorCourse(professorId, id));
        }
    }
