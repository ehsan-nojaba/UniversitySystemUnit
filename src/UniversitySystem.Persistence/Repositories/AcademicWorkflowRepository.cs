...
public sealed class AcademicWorkflowRepository(ApplicationDbContext _context) : IAcademicWorkflowRepository
{
    ...
    public Task<bool> ProfessorExistsAsync(long professorId, CancellationToken ct)
    {
        return _context.Professors.AnyAsync(p => p.Id == professorId && p.User.IsActive, ct);
    }
    public Task<long?> GetProfessorIdAsync(long userId, CancellationToken ct)
    {
        return _context.Professors.Where(p => p.UserId == userId && p.User.IsActive).Select(p => (long?)p.Id).SingleOrDefaultAsync(ct);
    }
    public Task<bool> TermExistsAsync(long termId, CancellationToken ct)
    {
        return _context.AcademicTerms.AnyAsync(t => t.Id == termId, ct);
    }
    public async Task<ICollection<long>> GetAllowedCourseIdsAsync(long professorId, CancellationToken ct)
    {
        return await _context.ProfessorCourses.AsNoTracking().Where(p => p.ProfessorId == professorId).Select(p => p.CourseId).ToListAsync(ct);
    }
    ...
    public async Task<bool> CoursesExistAsync(ICollection<long> ids, CancellationToken ct)
    {
        return await _context.Courses.CountAsync(c => ids.Contains(c.Id) && c.IsActive, ct) == ids.Count;
    }
    ...
    public Task<bool> HasSubmittedProposalAsync(long professorId, long courseId, long termId, CancellationToken ct)
    {
